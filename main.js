...
const addEvensOfPi = () => {
  let sumEvens = 0;
  let arr = stringsToNumbs(strNums)
  let evenArr = []
  for(elem of arr) {
    if (elem % 2 === 0) {
      evenArr.push(elem)
  ...
  sumEvens = evenArr.reduce((total, amount) => total + amount);
  return sumEvens
};

console.log(addEvensOfPi());
...
const weatherStates = (arr) => {
  let weatherArr = [];
  for(elem of arr) {
    // console.log(elem.weather_state_name)
